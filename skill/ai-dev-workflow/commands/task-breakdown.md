...
2. 每个任务格式：`[ID] [P?] [Phase] 描述（文件路径 + 验收方法）`
   - `[P]` = 可并行（不同文件、无依赖）
   - **完成标志（必填）**：每个任务的 验收 落成一行**可独立判定**的验证（命令/断言/curl/日志关键字即可判定，无需上下文；禁止"实现 XX"这类判不了对错的描述）——无上下文的新 subagent / 分批编码时靠它判定任务完成
...
- [ ] 每个任务有文件路径 + **可独立判定的完成标志**（命令/断言/curl/日志关键字即可判定；纯转发等"跳过测试"任务也写明完成标志）
