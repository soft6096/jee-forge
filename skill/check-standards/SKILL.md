...
6. 输出《关键规范核对报告》（标注核对范围；逐项 ✅/❌ + 证据 + 待用户确认清单），并落盘 5.2 规范核对报告（规范核对结论以本报告为准，不再汇总进 5.3 需求覆盖报告）
