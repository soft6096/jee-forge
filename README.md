...
└── skill/
    ...
    │   ├── templates/            # 21 份中间产物空白模板（0.0~5.3 + req-intake + 00-模块进度与断点）
...
>
> **模板命名规则**：`0.x / 1.x` 带小数点 = **流程步骤产物模板**（0.0 项目初始化、0.5 存量扫描、0.9 需求入口整形…5.3 验收）；`00-` 不带点 = **全程通用模板（非步骤）**：`00-中间产物-MD样式规范.md`（所有产物样式）、`00-模块进度与断点.md`（模块目录伴随，配合 `/progress`）。
...
| `/progress` | 进度/接续 | 读取/更新模块 `00-模块进度与断点.md`，报告每步状态与断点并从断点续跑（跨会话/换人接手先 `/progress`，禁止从 1.1 重跑） |
